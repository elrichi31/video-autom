...
export function generateCompositionTsx(script: VideoScript): string {
  const { slug } = script;
...
export const ${slug.replace(/-/g, "")}Duration = TOTAL;
...
/* ─── main ─── */
export const ${ComponentName}: FC<Props> = ({ voiceoverFile = null }) => {
  const off = {
    intro:   0,
    layers:  D.intro,
